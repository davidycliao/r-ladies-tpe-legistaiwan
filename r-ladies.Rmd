...
- 依屆期取得立法委員名單: get_ly_legislators_by_term()
...
- 使用者可以分析立法趨勢，如台灣民眾黨在不同會期的提案數量變化
...
library(stringr)
# 按會期、提案單位統計數量
bills_by_origin <- bills_term10 %>%
  group_by(會期, 提案單位) %>%
  summarise(count = n(), .groups = 'drop') %>%
  arrange(會期, desc(count))

# 篩選包含「黨團」的提案單位
bills_party <- bills_by_origin %>%
  filter(str_detect(提案單位, "黨團"))

# 進一步篩選「本院台灣民眾黨黨團」
minzhongdang_term10 <- bills_party %>%
...
ggplot(minzhongdang_term10, aes(x = 會期, y = count)) +
  ...
  scale_x_continuous(breaks = 
                       seq(min(minzhongdang_term10$會期),
...
p <- ggplot(attendees_count_40, aes(x = reorder(出席委員, count), y = count)) +
  geom_col(aes(fill = 出席委員), width = 0.7) +
  scale_fill_viridis_d(option = "turbo") +
  geom_text(aes(label = count), vjust = -0.5, size = 4, family = "Heiti TC", fontface = "bold") +  
...
## Case Study IV: Network Analysis based on Bill Co-occurrence (1/6)
